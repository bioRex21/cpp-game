...
void Sprite::init(char const *image1) {
  ...
  std::copy(VertexUtils::centeredImageVertices,
            VertexUtils::centeredImageVertices + 20, verts2);
  ...
  // param1: vertex attribute number, param2: size of vert attribute (vec3 = 3),
  // type, normalize, stride how many valies per vertex in our vertices[] array=
  // xyz (3) + colors(3) + tex coords or st (2)
  //  offset in the vertex [x,y,z,r,g,b,s,t]
  //                        ^               offset 0 for position (xyz)
  //                                ^       offset 3 for colors (rgb)
  //                                    ^   offset 6 for texture coordinates
  //                                    (st)
  //  position attribute
  ...
  // (void*)(3 * sizeof(float))); glEnableVertexAttribArray(1); texture coord
  // attribute
  ...
  glBindBuffer(
      GL_ARRAY_BUFFER,
      0);  // turn off saving state, we're done. Now can use it at render loop.
  ...
  this->ourShader =
      new BasicShader("basic_sprite_shader.vert", "basic_sprite_shader.frag");
...
void Sprite::render() {
  ...
  // model = glm::rotate(model, (float)glfwGetTime() * glm::radians(0.012f),
  ...
  // model = glm::rotate(model, (float)glfwGetTime() * glm::radians(15.012f),
